...
| PostgreSQL · MariaDB · SQL Server | `15438` · `13337` · `11437` |

## 2. Smoke test — los 8 criterios

### 1 — Las seis pantallas
Abra http://localhost:8015 y recorra el menú: Productos, Personas, Empresas,
Clientes, Vendedores y Facturas. Luego pruebe una que no existe:

```powershell
curl -i http://localhost:8015/noexiste      # → 404
```
...
```powershell
curl http://localhost:8009/api/persona/Z900
```
...
```powershell
curl http://localhost:8009/api/cliente
#  → "fkcodempresa": null
```
...
```powershell
curl -i http://localhost:8015/facturas/1/editar     # → 404
```
...
```powershell
curl http://localhost:8009/api/factura/1
