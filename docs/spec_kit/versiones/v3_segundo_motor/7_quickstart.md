...
```powershell
curl http://localhost:8009/     # → "version":"v3", "motor":"postgres"
```
...
curl http://localhost:8009/            # → "motor":"mariadb"
```
...
```powershell
curl -i http://localhost:8009/api/factura/999               # 404 "Factura 999 no existe"
...
| `mariadb` no queda healthy | Puerto 13337 ocupado, o volumen de un intento fallido: `docker compose down -v` y de nuevo |
