...
| Qué | Dónde |
|---|---|
| **EL FRONT** | http://localhost:8015 |
| La API — documentación | http://localhost:8009/docs |
| PostgreSQL | `localhost:15438` |
| MariaDB | `localhost:13337` |
| SQL Server | `localhost:11437` |
...
Abra http://localhost:8015 — debe redirigir al listado. Oprima
...
```powershell
curl http://localhost:8009/api/producto/P900
```
...
```powershell
curl -i http://localhost:8009/api/producto/P901    # → 404
```
...
```powershell
curl -i http://localhost:8009/api/producto/P900    # → 404
```
...
Recargue http://localhost:8015/productos. Debe ver:
...
```powershell
curl http://localhost:8009/api/producto
curl http://localhost:8009/api/persona
curl "http://localhost:8009/api/factura?limite=5"
```
...
| `Connection refused` al abrir 8015 | El front no arrancó: `docker compose logs front-flask` |
