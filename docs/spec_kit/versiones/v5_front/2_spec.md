...
Jinja2 (`front_flask/`, puerto **8015**) que habla con la API por HTTP y
...
- `front_flask/` como servicio propio del compose, en el puerto 8015.
...
1. **Un solo comando.** `docker compose up -d --build` deja arriba los seis
   servicios. `http://localhost:8015` redirige al listado y responde 200.
