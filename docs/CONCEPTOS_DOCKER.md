...
| `EXPOSE 8009` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
- En la v1: `bd_v1` es un contenedor creado desde la imagen `postgres:16-alpine`,
  con el puerto interno 5432 **publicado** en el 15438 de su PC (`-p 15438:5432`).
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTFLASK["SERVIDOR WEB (front)<br/>contenedor front-flask<br/>hostname: front-flask · escucha en 8015"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8009"]
    ...
    NAV -->|"localhost:8009"| APIFACTURAS
    NAV -->|"localhost:8015"| FRONTFLASK
    ...
    FRONTFLASK -->|"http://api-facturas:8009<br/>al controlador, por el NOMBRE"| APIFACTURAS
    SQLSERVERINIT -->|"espera el healthcheck,<br/>siembra y termina"| SQLSERVER
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13337"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15438"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11437"| SQLSERVER
```
...
services:                          # el mapa de TODOS los contenedores del sistema
  ...
  postgres:                        # ← este nombre es también su HOSTNAME interno
    ...
    ports:
      - "15438:5432"               # "puerto en su PC : puerto interno del contenedor"
  ...
  api-facturas:
    ...
    ports:
      - "8009:8009"                # http://localhost:8009/docs
    environment:
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8009`, `localhost:15438`); entre contenedores, nombres de
...
docker network create proyecto_paradigmas6_default

# 2. postgres
docker run -d --name postgres --network proyecto_paradigmas6_default --restart unless-stopped `
  -e "POSTGRES_DB=bdfacturas_postgres_local" `
  -e "POSTGRES_USER=paradigmas" `
  -e "POSTGRES_PASSWORD=paradigmas123" `
  -v pgdata:/var/lib/postgresql/data `
  -v "${PWD}/db/init.sql:/docker-entrypoint-initdb.d/init.sql:ro" `
  -p 15438:5432 postgres:16-alpine

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. mariadb
docker run -d --name mariadb --network proyecto_paradigmas6_default --restart unless-stopped `
  ...
  -p 13337:3306 mariadb:11

# 5. ESPERAR a que responda de verdad… mirándolo a ojo

# 6. sqlserver
docker run -d --name sqlserver --network proyecto_paradigmas6_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Paradigmas123!" `
  -e "MSSQL_PID=Developer" `
  -v mssqldata:/var/opt/mssql `
  -p 11437:1433 mcr.microsoft.com/mssql/server:2022-latest

# 7. ESPERAR a que responda de verdad… mirándolo a ojo

# 8. sqlserver-init
docker run -d --name sqlserver-init --network proyecto_paradigmas6_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Paradigmas123!" `
...
docker run -d --name api-facturas --network proyecto_paradigmas6_default --restart unless-stopped `
  ...
  -p 8009:8009 api-facturas uvicorn main:app --host 0.0.0.0 --port 8009 --reload

# 10. Construir la imagen de front-flask y encenderla
docker build -t front-flask ./front_flask
docker run -d --name front-flask --network proyecto_paradigmas6_default --restart unless-stopped `
  -e "API_FACTURAS_URL=http://api-facturas:8009" `
  -e "CLAVE_SESION=Paradigmas123!Sesion" `
  -v "${PWD}/front_flask:/app" `
  -p 8015:8015 front-flask
...
```yaml
URL_API: http://api-facturas:8009
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8009` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8009` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8009:8009`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8009`, el front
...
```powershell
docker network inspect proyecto_paradigmas6_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_paradigmas6` produce
la red `proyecto_paradigmas6_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_paradigmas6-front-flask-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_paradigmas6_default has active endpoints (name:"proyecto_paradigmas6-front-flask-1" …)
```
