...
PUERTO = int(os.environ.get("PUERTO", 8015))


@app.context_processor
def _menu():
    """El menú de navegación sale de la misma declaración, no de una lista
    escrita aparte que habría que acordarse de actualizar."""
    return {"entidades": ENTIDADES.values()}
...
def _opciones(definicion):
    ...
    for campo in definicion.campos:
        if campo.opciones_de:
            ok, filas, _ = cliente_api.RECURSOS[campo.opciones_de].listar()
...
def listar(entidad):
    ...
    ok, filas, errores = cliente_api.RECURSOS[entidad].listar()
    if not ok:
        _avisar(errores)
...
def crear(entidad):
    ...
    ok, errores = cliente_api.RECURSOS[entidad].crear(datos)
    if ok:
...
def editar(entidad, llave):
    definicion = _entidad(entidad)
    recurso = cliente_api.RECURSOS[entidad]
...
def eliminar(entidad, llave):
    ...
    definicion = _entidad(entidad)
    ok, errores = cliente_api.RECURSOS[entidad].eliminar(llave)
    if ok:
...
def nueva_factura():
    ...
    def _catalogos():
        return {
            "clientes": cliente_api.RECURSOS["cliente"].listar()[1],
            "vendedores": cliente_api.RECURSOS["vendedor"].listar()[1],
            "productos": cliente_api.RECURSOS["producto"].listar()[1],
        }
